...
logit_sim <- function(beta_vector, n, runs, seed = 1234, dirty = 0, type){
  ...
  # Making model list to save the models
  model_list <- vector("list", runs)  
  
  # Sample sizes for clean and dirty data
  ...
  for(r in 1:runs){
    ...
    X_matrix[, 1] <- rep(1, n_clean) # Intercept = 1-vector
    for(j in 2:p){
      X_matrix[, j] <- rnorm(n = n_clean) # Standardnormal randoms
    }
    
    # Generating outcome
    eta_vector  <- X_matrix %*% beta_vector  # Linear predictor (eta)
    prob_vector <- exp(eta_vector)/(1 + exp(eta_vector)) # expected outcome (E(y = 1)) = PROBABILITY
...
Putting the data generation in a separate function:
```{r}
