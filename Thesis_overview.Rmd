...
logit_sim <- function(beta_vector, n, p_clean, runs, seed){
  ...
  for(r in 1:runs){
    ...
    X_matrix[, 1] <- rep(1, n)
    for(j in 2:p_clean){
      X_matrix[, j] <- runif(n = n, min = 0, max = 1)
    }
    
    # Generating outcome
    n_dirty <- round(0.20 * n, 0)
    n_clean <- n - n_dirty
    
    beta_dirty <- c(-0.5, 0.8, 1.3)
    
    eta_vector <- X_matrix[1:n_clean, ] %*% beta_vector
    eta_dirty <- X_matrix[(n_clean + 1):n, ] %*% beta_dirty
    pi_vector <- 1/(1 + exp(-eta_vector))
    pi_dirty <- 1/(1 + exp(-eta_dirty))
    y_vector <- rbinom(n = n_clean,
                       size = 1,
                       prob = pi_vector)
    y_dirty <- rbinom(n = n_dirty,
                       size = 1,
                       prob = pi_dirty)
    ...
    data <- as.data.frame(cbind(y, X_matrix[, -1]))
...
plot(X_matrix[ ,2], y_vector)

```

Running simulation
```{r}
beta_vector <- c(0.5, -1.3, 1.7)
test1 <- logit_sim(beta_vector = beta_vector, n = 500, runs = 1999)

coefs <- t(sapply(test1, coef))
# Avg
colMeans(coefs)

# MSE:
colMeans((coefs - beta_vector)^2)
