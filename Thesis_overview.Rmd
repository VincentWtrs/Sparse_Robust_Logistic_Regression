...
# Linear Regression
See file: linear_model_example.Rmd
...
If we want multiple runs of such model (i.e. simulation), we need to run it multiple times, now we will automate this using a very simple function (which we will not use much further on, I suppose) and then using `replicate()` on this function. The function will generate data and specify the DGP and fit a logit model, then get the coefficients. By replicating this many times, due to the slightly different data (X) every time being sampled (so called sampling variability) __and__ also due to sampling from the binomial distribution (i.e. even with the exact same probability parameter we ofcourse get different realizations). As an example we will have 2 predictors and an intercept.

Note: the function is very basic, i.e. the amount of predictors can not be varied, the structure of the predictor data cannot be altered...

```{r defining_temporary_logitcoefs_simulation_function}

## Defining simulation + 
logitcoefs <- function(N = 1000, beta0 = 1, beta1 = 2, beta2 = 3) {
  # Sample size
  N <- N
  ...
  x1 <- rnorm(N) # This distribution doesn't matter
  x2 <- rnorm(N) # This distribution doesn't matter
...
Using the function and replicating the simulation `reps` times. If `reps` is big enough, e.g. 200 times or higher, the estimators are behaving well and the central limit theorem kicks in nicely and the estimators are close to their asymptotic distributions. But ofcourse this is probably (to do: check better) with the sample sizes used individually. The histograms show this (red line: means over replications).
```{r replicating_logit_coefs()_function}
# Replicating and getting results:
reps = 1000
results <- t(replicate(n = reps, 
...
for(i in 1:ncol(results)){
  hist(results[, i],
       freq = FALSE,
       main = paste0("Histogram of beta_hat ", i),
       xlab = paste0("beta_hat ", i))
  abline(v = colMeans(results)[i], col = "red")
...
* `$objective`: sum of the __h penalized__ deviances (see ´$hsize` and `$``)
...
WHAT ABOUT SOMEKIND OF AUC FOR BETA RECOVERY (is also 2 outcome thing) or confusion matrix (Yes see: Bühlmann & Li, 2003)
