...
We simply use the DGP functions from before, set the dimensionality to be higher, amount of contamination to 0 and run some fitting functions such as `glm()` and `glmnet()`.


```{r}
# Creating training set (clean in this case)
...
loss_comparison1 <- cbind(ridge_logit1_loss$loss, lasso_logit1_loss$loss, enet_logit1_loss$loss)
colnames(loss_comparison1) <- c("lasso", "ridge", "alpha = 0.75")
...
__To do: continue the documentation__
...
# Creating a test set (clean data!)
test1 <- binary_reg_dgp2(n = 100,
                          p = 50,
                          p_a = 5,
                          beta = c(rep(1, 5), rep(0, 45)),
...
## Fitting enetLTS: different options
# DEFAULT enetLTS
enetLTS1 <- enetLTS(xx = train1[, -1],
                    yy = train1[, 1],
                    family = "binomial")

# enetLTS: User supplied alpha/lambdas
enetLTS2 <- enetLTS(xx = train1[, -1],
                    yy = train1[, 1],
                    family = "binomial",
