beginningCstep_ANNOTATED <- function (x, y, family, h, hsize, alpha, lambda, nsamp, s1, ncores, csteps, tol, scal, para, seed) 
{
  ...
  if (para) {
    lastbestindex <- parallel:::mclapply(1:s1, function(zz, x, y, family, h, hsize, alpha, lambda, H2) { ## DEFINNG FUNCTION TO BE RAN WITHIN MCLAPPLY
      indexsubbest <- H2$idxbest[[zz]]
      objbest <- tol
      cstep.mod <- enetLTS:::CStep(x, y, family, indexsubbest, h, hsize, alpha, lambda/h, scal) # Running a C-step
